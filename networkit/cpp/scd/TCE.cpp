...
TCE::TCE(const Graph& G, bool refine, bool useJaccard) : SelectiveCommunityDetector(G), refine(refine), useJaccard(useJaccard) {
}
 ...
 */
double weightedEdgeScore(const Graph&G, node, node v, edgeweight uvWeight, double uDegree, const std::unordered_map<node, double>& uNeighbors, bool useJaccard) {
	double nom = uvWeight;
	double vDegree = 0.0;
...
std::set<node> expandSeedSet_internal(const Graph&G, const std::set<node>& s, bool refine, bool useJaccard) {
	// global data structures
	std::set<node> result = s;

	std::vector<node> local_to_global_id;
	std::unordered_map<node, node> global_to_local_id;
	//global_to_local_id.set_empty_key(none);
 ...
 // 	};
	...
	auto updateShell = [&] (node u, bool noverify = false) -> double {
		if (G.degree(u) == 0.0)
			return 0.0;
...
#ifndef NDEBUG
				assert(score_uv == weightedEdgeScore(G, u, local_to_global_id[v], weight, xDegree, uNeighbors, useJaccard));

				if (shell.contains(v)) {
					assert(shell.getKey(v) == node_score[v]);
				}
#endif
				node_score[v] += score_uv;
				shell.pushOrIncreaseKey(v, node_score[v]);
...
//		auto avgIntDeg = [](count n_nodes, double volume, double cut) {
//			return (volume - cut) / n_nodes;
//		};
//		if (avgIntDeg(result.size(), volume, numCutEdges) < avgIntDeg(result.size() + 1, volumeNew, numCutEdgesNew)) {
