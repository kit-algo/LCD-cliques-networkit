...
class LocalCommunity {
	static_assert(!MaintainBoundary || ShellMaintainsExtDeg, "boundary cannot be maintained without maintaining the external degree");
public:
	...
	struct OptionalValue {
		...
		void set(ValueType) {
			throw std::runtime_error("Setting value that is missing");
		}
	...
	struct CommunityInfo {
		...
		CommunityInfo() : intDeg(0), extDeg(0), exclusiveOutsideNeighbor(none), numFullyInternalNeighbors(0) {}
	};
