#include "LocalCommunity.h"

namespace NetworKit {
	template <bool ShellMaintainsExtDeg, bool MaintainBoundary, bool AllowRemoval>
	LocalCommunity<ShellMaintainsExtDeg, MaintainBoundary, AllowRemoval>::LocalCommunity(const Graph& G) : G(G), intWeight(0), extWeight(0) {}
	...
	void LocalCommunity<ShellMaintainsExtDeg, MaintainBoundary, AllowRemoval>::addNode(node u) {
		...
		node boundaryNeighbor = none; // if u is in the boundary and has only one neighbor outside of the community, store it here.
	...
	std::unordered_set<node> LocalCommunity<a, b, c>::calculateBoundary() {
	...
	template <bool a, bool b, bool c>
	std::pair<double, double> LocalCommunity<a, b, c>::calculateIntExtDeg(node v) {
		double degInt = 0;
		double degExt = 0;
	...
	template <bool a, bool b, bool c>
	std::pair<double, double> LocalCommunity<a, b, c>::calculateVolumeCut() {
		double internal = 0;
		double external = 0;
		for (const auto& it : community) {
