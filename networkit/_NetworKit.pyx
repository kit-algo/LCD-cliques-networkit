...
cdef extern from "cpp/scd/SelectiveCommunityDetector.h":
...
cdef class SelectiveCommunityDetector:
	...
	def expandOneCommunity(self, seeds):
		...
		"""
		if isinstance(seeds, collections.Iterable):
...
cdef extern from "cpp/scd/SCDGroundTruthComparison.h":
	cdef cppclass _SCDGroundTruthComparison "NetworKit::SCDGroundTruthComparison"(_Algorithm):
		_SCDGroundTruthComparison(_Graph G, _Cover groundTruth, map[node, set[node]] found, bool ignoreSeeds) except +
...
cdef extern from "cpp/scd/LFMLocal.h":
...
cdef extern from "cpp/scd/RandomBFS.h":
...
cdef extern from "cpp/scd/TwoPhaseL.h":
...
cdef extern from "cpp/scd/LocalTightnessExpansion.h":
...
cdef extern from "cpp/scd/TCE.h":
	cdef cppclass _TCE "NetworKit::TCE"(_SelectiveCommunityDetector):
		_TCE(_Graph G, bool refine, bool useJaccard) except +
...
cdef class TCE(SelectiveCommunityDetector):
	...
	"""
	def __cinit__(self, Graph G, bool refine = True, bool useJaccard = False):
		self._G = G
		self._this = new _TCE(G._this, refine, useJaccard)

cdef extern from "cpp/scd/LocalT.h":
